...
int main(int argc, char *argv[]) {
  ...
  for (int i = 2; i < argc; i++) {
    if (argv[i][0] == '-') // feature switches
    {
      ...
      case 'M':
        mp4fuzzchanges = atoi(&argv[i][2]);
        break;
      case 'G':
        gpmffuzzchanges = atoi(&argv[i][2]);
        break;
      case 'F':
        fuzzloopcount = atoi(&argv[i][2]);
        break;
      }
    }
  }
  ...
  do {
    ...
    if (fuzzloopcount)
      printf("%5d/%5d\b\b\b\b\b\b\b\b\b\b\b",
             resetfuzzloopcount - fuzzloopcount + 1, resetfuzzloopcount);
  ...
  if (fpr) {
    ...
    if (fpw) {
      for (pos = 0; pos < len;) {
        ...
        if (pos >= (len - 120000)) {
          srand(mp4fuzzchanges * resetfuzzloopcount +
                (resetfuzzloopcount - fuzzloopcount) + gpmffuzzchanges);
          int times;
...
GPMF_ERR readMP4File(char *filename) {
  ...
  if (mp4fuzzchanges) {
    ...
    mp4handle =
        OpenMP4Source(filename, MOV_GPMF_TRAK_TYPE, MOV_GPMF_TRAK_SUBTYPE, 0);
    if (mp4handle == 0)
      return GPMF_OK; // when fuzzing, errors reported are showing the system is
                      // working.
  }

  metadatalength = GetDuration(mp4handle);

  printf("VIDEO DURATION: %.3f seconds\n", GetVideoDuration(mp4handle));
            ...
            if (GPMF_OK == ret) // find the last FOURCC within the stream
            {
              ...
              if (samples) {
                ...
                if (type == GPMF_TYPE_COMPLEX) {
                  ...
                  if (GPMF_OK ==
                      GPMF_FindPrev(&find_stream, GPMF_KEY_TYPE,
                                    GPMF_CURRENT_LEVEL | GPMF_TOLERANT)) {
                    char tmp[64];
...
#define MAX_UNITLEN 8
              ...
              if (tmpbuffer) {
                ...
                GPMF_CopyState(ms, &find_stream);
                if (GPMF_OK ==
                        GPMF_FindPrev(&find_stream, GPMF_KEY_SI_UNITS,
                                      GPMF_CURRENT_LEVEL | GPMF_TOLERANT) ||
                    GPMF_OK ==
                        GPMF_FindPrev(&find_stream, GPMF_KEY_UNITS,
                                      GPMF_CURRENT_LEVEL | GPMF_TOLERANT)) {
                  char *data = (char *)GPMF_RawData(&find_stream);
