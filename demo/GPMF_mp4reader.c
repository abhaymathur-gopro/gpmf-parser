...
/* This is not an elegant MP4 parser, only used to help demonstrate extraction
 * of GPMF */
...
size_t GetPayloadResource(size_t mp4handle, size_t resHandle,
...
uint32_t *GetPayload(size_t mp4handle, size_t resHandle, uint32_t index) {
  ...
  if (index < mp4->indexcount && mp4->mediafp) {
    if ((mp4->filesize >= mp4->metaoffsets[index] + mp4->metasizes[index]) &&
        (mp4->metasizes[index] > 0)) {
      uint32_t buffsizeneeded =
          mp4->metasizes[index]; // Add a little more to limit reallocations
...
uint32_t WritePayload(size_t handle, uint32_t *payload, uint32_t payloadsize,
                      uint32_t index) {
  ...
  if (index < mp4->indexcount && mp4->mediafp) {
    if ((mp4->filesize >= mp4->metaoffsets[index] + mp4->metasizes[index]) &&
        mp4->metasizes[index] == payloadsize) {
#ifdef _WINDOWS
      _fseeki64(mp4->mediafp, (__int64)mp4->metaoffsets[index], SEEK_SET);
#else
      fseeko(mp4->mediafp, (off_t)mp4->metaoffsets[index], SEEK_SET);
...
uint32_t GetPayloadSize(size_t handle, uint32_t index) {
  ...
  if (mp4->metasizes && mp4->metasize_count > index)
    return mp4->metasizes[index] &
           (uint32_t)~0x3; // All GPMF payloads are 32-bit aligned and sized
...
size_t OpenMP4Source(char *filename, uint32_t traktype, uint32_t traksubtype,
                     int32_t flags) // RAW or within MP4
{
    ...
    do {
      ...
      if (len == 8 && mp4->filepos < mp4->filesize) {
        ...
        if (!VALID_FOURCC(qttag) &&
            (qttag & 0xff) != 0xa9) // ©xyz and ©swr are allowed
        {
          CloseSource((size_t)mp4);
          mp4 = NULL;
          break;
        ...
        if (qtsize - len >
            (mp4->filesize - mp4->filepos)) // not parser truncated files.
        {
          CloseSource((size_t)mp4);
          mp4 = NULL;
          break;
        ...
        if (nest >= MAX_NEST_LEVEL)
          break;
        if (nest > 1 && qtsize > nestsize[nest - 1])
          break;
        ...
        for (int i = 1; i < nest; i++)
          printf("%5d ", nestsize[i]); // printf("    ");
        printf(" %c%c%c%c (%lld)\n", (qttag & 0xff), ((qttag >> 8) & 0xff),
               ((qttag >> 16) & 0xff), ((qttag >> 24) & 0xff), qtsize);
...
#endif
        if (qttag !=
                MAKEID('m', 'o', 'o', 'v') && // skip over all but these atoms
          ...
          } else if (qttag == MAKEID('m', 'v', 'h', 'd')) // mvhd  movie header
          {
            ...
            mp4->clockcount = BYTESWAP32(mp4->clockcount);
          ...
          } else if (qttag == MAKEID('m', 'd', 'h', 'd')) // mdhd  media header
          {
            ...
            if (len == sizeof(md)) {
              ...
              if (mp4->videolength ==
                  0.0) // Get the video length from the first track
              {
                mp4->videolength = (float)((double)mp4->trak_clockcount /
                                           (double)mp4->trak_clockdemon);
              }
            }
          ...
          } else if (qttag == MAKEID('h', 'd', 'l', 'r')) // hldr
          {
            ...
            len += fread(
                &temp, 1, 4,
                mp4->mediafp); // type will be 'meta' for the correct trak.
          ...
          } else if (qttag == MAKEID('e', 'd', 't', 's')) // edit list
          {
            ...
            if (elst == MAKEID('e', 'l', 's', 't')) {
              ...
              if (temp == 0) {
                ...
                if (readnum <= (qtsize / 12) && mp4->trak_clockdemon) {
                  uint32_t
                      segment_duration; // integer that specifies the duration
                                        // of this edit segment in units of the
                                        // movies time scale.
                  uint32_t
                      segment_mediaTime; // integer containing the starting time
                                         // within the media of this edit
                                         // segment(in media timescale units).If
                                         // this field is set to 1, it is an
                                         // empty edit.The last edit in a track
                                         // should never be an empty edit.Any
                                         // difference between the movies
                                         // duration and the tracks duration
                                         // is expressed as an implicit empty
                                         // edit.
                  uint32_t segment_mediaRate; // point number that specifies the
                                              // relative rate at which to play
                                              // the media corresponding to this
                                              // edit segment.This rate value
                  ...
                  for (i = 0; i < readnum; i++) {
                    ...
                    segment_duration =
                        BYTESWAP32(segment_duration); // in MP4 clock base
                    segment_mediaTime =
                        BYTESWAP32(segment_mediaTime); // in trak clock base
                    segment_mediaRate = BYTESWAP32(
                        segment_mediaRate); // Fixed-point 65536 = 1.0X

                    if (segment_mediaTime ==
                        0xffffffff) // the segment_duration for blanked time
                      mp4->trak_edit_list_offsets[mp4->trak_num] += (int32_t)
                          segment_duration; // samples are delay, data starts
                                            // after presentation time zero.
                    else if (i == 0) // If the first editlst starts after zero,
                                     // the track is offset by this time (time
                                     // before presentation time zero.)
                      mp4->trak_edit_list_offsets[mp4->trak_num] -=
                          (int32_t)((double)segment_mediaTime /
                                    (double)mp4->trak_clockdemon *
                                    (double)mp4->clockdemon); // convert to MP4
                                                              // clock base.
                  }
                  if (type == traktype) // GPMF metadata
                  {
                    mp4->metadataoffset_clockcount =
                        mp4->trak_edit_list_offsets
                            [mp4->trak_num]; // leave in MP4 clock base
                  }
                }
              }
            }
            ...
            NESTSIZE(qtsize);
          } else if (qttag == MAKEID('s', 't', 's',
                                     'd')) // read the sample decription to
                                     ...
                                     'c')) // metadata stsc - offset chunks
          {
            if (type == traktype) // meta
            {
              ...
              if (num <= (qtsize / sizeof(SampleToChunk))) {
                ...
                if (num > 0 && qtsize > (num * sizeof(SampleToChunk))) {
                  mp4->metastsc =
                      (SampleToChunk *)malloc(num * sizeof(SampleToChunk));
                  if (mp4->metastsc) {
                    len += fread(mp4->metastsc, 1, num * sizeof(SampleToChunk),
                                 mp4->mediafp);

                    do {
                      num--;
                      mp4->metastsc[num].chunk_num =
                          BYTESWAP32(mp4->metastsc[num].chunk_num);
                      mp4->metastsc[num].samples =
                          BYTESWAP32(mp4->metastsc[num].samples);
            ...
            NESTSIZE(qtsize);
                     ...
                     MAKEID('s', 't', 's', 'z')) // metadata stsz - sizes
          {
            if (type == traktype) // meta
            {
              ...
              num = BYTESWAP32(num);
              // if equalsamplesize != 0, it is the size of all the samples and
              // the length should be 20
              // (size,fourcc,flags,samplesize,samplecount)
              if (qtsize >= (20 + (num * sizeof(uint32_t))) ||
                  (equalsamplesize != 0 && qtsize == 20)) {
                if (mp4->metasizes) {
                  free(mp4->metasizes);
                  mp4->metasizes = 0;
                }
                // either the samples are different sizes or they are all the
                // same size
                if (num > 0 &&
                    num < 5184000) // number of frame in 24hours at 60fps (crude
                                   // limiter for corrupted num data.)
                {
            ...
            NESTSIZE(qtsize);
                     ...
                     MAKEID('s', 't', 'c', 'o')) // metadata stco - offsets
          {
            if (type == traktype) // meta
            {
              len = fread(&skip, 1, 4, mp4->mediafp);
              ...
              if (num <= ((qtsize - 8 - len) / sizeof(uint32_t))) {
                ...
                if (mp4->metastsc_count > 0 && mp4->metasize_count > 0 &&
                    num != mp4->metasize_count) {
                  if (mp4->metaoffsets) {
                  ...
                  if (mp4->metastco_count && qtsize > (num * 4) &&
                      num < 5184000 &&
                      mp4->metasize_count <
                          5184000) // number of frame in 24hours at 60fps (crude
                                   // limiter for corrupted num data.)
                  {
                    mp4->metaoffsets =
                        (uint64_t *)malloc(mp4->metasize_count * 8);
                    if (mp4->metaoffsets) {
                      ...
                      if (metaoffsets32) {
                        ...
                        while (num < mp4->metasize_count) {
                          if (mp4->metastsc[stsc_pos].samples &&
                              num != mp4->metastsc[stsc_pos].chunk_num - 1 &&
                              0 == (num -
                                    (mp4->metastsc[stsc_pos].chunk_num - 1)) %
                                       mp4->metastsc[stsc_pos].samples) {
                            stco_pos++;
                          ...
                          // int delta = metaoffsets[num] - metaoffsets[num -
                          // 1]; printf("%3d:%08x, delta = %08x\n", num,
                          // (int)fileoffset, delta);
                  ...
                  if (num > 0 && mp4->metasize_count && mp4->metasizes &&
                      qtsize > (num * 4) &&
                      num < 5184000) // number of frame in 24hours at 60fps
                                     // (crude limiter for corrupted num data.)
                  {
                    ...
                    if (mp4->metaoffsets) {
                      ...
                      if (metaoffsets32) {
                        size_t readlen =
                            fread(metaoffsets32, 1, num * 4, mp4->mediafp);
                        len += readlen;
                        do {
                          num--;
                          mp4->metaoffsets[num] =
                              BYTESWAP32(metaoffsets32[num]);
                        } while (num > 0);
            ...
            NESTSIZE(qtsize);
                   ...
                   MAKEID('c', 'o', '6', '4')) // metadata stco - offsets
          {
            if (type == traktype) // meta
            {
              len = fread(&skip, 1, 4, mp4->mediafp);
                               ...
                               // limiter for corrupted num data.)
              {
                ...
                if (mp4->metastsc_count > 0 && num != mp4->metasize_count) {
                  ...
                  if (mp4->metasize_count && mp4->metasizes &&
                      qtsize > (num * 8)) {
                    mp4->metaoffsets =
                        (uint64_t *)malloc(mp4->metasize_count * 8);
                    if (mp4->metaoffsets) {
                  ...
                  if (num > 0 && mp4->metasize_count && mp4->metasizes &&
                    ...
                    if (mp4->metaoffsets) {
                      ...
                      do {
                        num--;
                        mp4->metaoffsets[num] =
                            BYTESWAP64(mp4->metaoffsets[num]);
                      } while (num > 0);
                    }
                  }
                }
          ...
          {
            if (type ==
                MAKEID('v', 'i', 'd', 'e')) // video trak to get frame rate
            {
              uint32_t samples = 0;
              uint32_t entries = 0;
              len = fread(&skip, 1, 4, mp4->mediafp);
            ...
            } else if (type == traktype)       // meta
            {
              ...
              if (num <= (qtsize / 8)) {
                ...
                while (entries > 0) {
                  ...
                  totaldur += duration;
                  mp4->metadatalength +=
                      (double)((double)samplecount * (double)duration /
                               (double)mp4->meta_clockdemon);
                  if (samplecount > 1 || num == 1 ||
                      mp4->basemetadataduration == 0.0)
                    mp4->basemetadataduration = mp4->metadatalength *
                                                (double)mp4->meta_clockdemon /
                                                (double)samples;
                }
              }
...
float GetVideoDuration(size_t handle) {
  mp4object *mp4 = (mp4object *)handle;
  if (mp4 == NULL)
    return 0.0;

  return (float)mp4->videolength;
}

uint32_t GetVideoFrameRateAndCount(size_t handle, uint32_t *numer,
                                   uint32_t *demon) {
  mp4object *mp4 = (mp4object *)handle;
  if (mp4 == NULL)
    return 0;
...
uint32_t GetPayloadTime(size_t handle, uint32_t index, double *in,
                        double *out) {
  mp4object *mp4 = (mp4object *)handle;
  if (mp4 == NULL)
    return MP4_ERROR_MEMORY;

  if (mp4->metaoffsets == 0 || mp4->basemetadataduration == 0 ||
      mp4->meta_clockdemon == 0 || in == NULL || out == NULL)
    return MP4_ERROR_MEMORY;

  *in = ((double)index * (double)mp4->basemetadataduration /
         (double)mp4->meta_clockdemon);
  *out = ((double)(index + 1) * (double)mp4->basemetadataduration /
          (double)mp4->meta_clockdemon);
...
uint32_t GetPayloadRationalTime(size_t handle, uint32_t index,
                                int32_t *in_numerator, int32_t *out_numerator,
  ...
  if (mp4->metaoffsets == 0 || mp4->basemetadataduration == 0 ||
      mp4->meta_clockdemon == 0 || in_numerator == NULL ||
      out_numerator == NULL)
    return MP4_ERROR_MEMORY;
  ...
  *out_numerator = (int32_t)((index + 1) * mp4->basemetadataduration);
  ...
  if (*out_numerator >
      (int32_t)((double)mp4->metadatalength * (double)mp4->meta_clockdemon))
    *out_numerator =
        (int32_t)((double)mp4->metadatalength * (double)mp4->meta_clockdemon);

  // Add any Edit List offset
  *in_numerator += (int32_t)(((double)mp4->metadataoffset_clockcount /
                              (double)mp4->clockdemon) *
                             mp4->meta_clockdemon);
  *out_numerator += (int32_t)(((double)mp4->metadataoffset_clockcount /
                               (double)mp4->clockdemon) *
                              mp4->meta_clockdemon);
...
uint32_t GetEditListOffsetRationalTime(size_t handle, int32_t *offset_numerator,
                                       uint32_t *denominator) {
  mp4object *mp4 = (mp4object *)handle;
  if (mp4 == NULL)
    return MP4_ERROR_MEMORY;
    ...
    do {
      ...
      if (len == 8) {
        ...
        if (qttag == MAKEID('G', 'P', 'M', 'F')) {
          ...
          mp4->metasizes = (uint32_t *)malloc(mp4->indexcount * 4 + 4);
          memset(mp4->metasizes, 0, mp4->indexcount * 4 + 4);
          mp4->metaoffsets = (uint64_t *)malloc(mp4->indexcount * 8 + 8);
          memset(mp4->metaoffsets, 0, mp4->indexcount * 8 + 8);
          ...
          return (size_t)mp4; // not an MP4, RAW GPMF which has not inherent
                              // timing, assigning a during of 1second.
        }
        if (qttag !=
                MAKEID('m', 'o', 'o', 'v') && // skip over all but these atoms
            qttag != MAKEID('u', 'd', 't', 'a')) {
